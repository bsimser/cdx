...
namespace CdxLib.Physics.Graphics
{
    public class AssetCreator
    {
        ...
        private readonly Dictionary<MaterialType, Texture2D> _materials = new Dictionary<MaterialType, Texture2D>();
        ...
        public static Vector2 CalculateOrigin(Body b)
        {
            ...
            foreach (var t in b.FixtureList)
            {
                for (var j = 0; j < t.Shape.ChildCount; ++j)
                {
                    AABB bounds;
                    t.Shape.ComputeAABB(out bounds, ref trans, j);
        ...
        public Texture2D TextureFromVertices(Vertices vertices, MaterialType type, Color color, float materialScale)
        {
            ...
            var decomposedVerts = !verts.IsConvex() ? EarclipDecomposer.ConvexPartition(verts) : new List<Vertices> {verts};
            var verticesFill =
        ...
        public Texture2D EllipseTexture(float radiusX, float radiusY, MaterialType type, Color color,
                                        float materialScale)
        {
            var verticesFill = new VertexPositionColorTexture[3*(CircleSegments - 2)];
            var verticesOutline = new VertexPositionColor[2*CircleSegments];
            const float segmentSize = MathHelper.TwoPi/CircleSegments;
            var theta = segmentSize;

            radiusX = ConvertUnits.ToDisplayUnits(radiusX);
            radiusY = ConvertUnits.ToDisplayUnits(radiusY);
            materialScale /= _materials[type].Width;

            var start = new Vector2(radiusX, 0f);

            for (var i = 0; i < CircleSegments - 2; ++i)
            {
                var p1 = new Vector2(radiusX*(float) Math.Cos(theta), radiusY*(float) Math.Sin(theta));
                var p2 = new Vector2(radiusX*(float) Math.Cos(theta + segmentSize),
        ...
        /// <param name = "verticesFill"></param>
        /// <param name = "verticesOutline"></param>
        /// <returns></returns>
        ...
        {
            var fill = new List<VertexPositionColorTexture[]>(1) {verticesFill};
        ...
        /// <returns></returns>
        private Texture2D RenderTexture(int width, int height, Texture2D material,
                                        IEnumerable<VertexPositionColorTexture[]> verticesFill,
                                        VertexPositionColor[] verticesOutline)
        {
            var halfPixelOffset = Matrix.CreateTranslation(-0.5f, -0.5f, 0f);
            var pp = _device.PresentationParameters;
            var texture = new RenderTarget2D(_device, width + 2, height + 2, false, SurfaceFormat.Color,
            ...
            foreach (var t in verticesFill)
            {
                _device.DrawUserPrimitives(PrimitiveType.TriangleList, t, 0, t.Length/3);
            }
            // render outline;
