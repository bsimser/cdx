...
namespace CdxLib.Core.Controls
{
    ...
    public class PageFlipTracker
    {
        ...
        private float _flipStartOffset;
        ...
        {
            get { return PageWidthList.Count >= 2 && CurrentPageOffset > 0; }
        }
        ...
        public bool IsRightPageVisible
        {
            get
            {
                return PageWidthList.Count >= 2 &&
                       CurrentPageOffset + EffectivePageWidth(CurrentPage) <= TouchPanel.DisplayWidth;
            }
        }
        ...
        {
            var displayWidth = TouchPanel.DisplayWidth - PreviewMargin;
        ...
        {
            if (!InFlip) return;
            var transitionClock = DateTime.Now - _flipStartTime;
            ...
            foreach (var sample in input.Gestures)
            {
                switch (sample.GestureType)
                {
                    case GestureType.HorizontalDrag:
                        ...
                        _flipStartOffset = CurrentPageOffset;
                        break;
                    ...
                    case GestureType.DragComplete:
                        if (!InFlip)
                        {
                            ...
                            else if (CurrentPageOffset + TouchPanel.DisplayWidth*(1 - DragToFlipTheshold) >
                                     EffectivePageWidth(CurrentPage))
                            {
